...
mnemonic.mn_get_checksum_index = function (words, prefix_len) {
    var trimmed_words = "";
...
mnemonic.mn_encode = function (str, wordset_name) {
    ...
    for (var j = 0; j < str.length; j += 8) {
        str = str.slice(0, j) + this.mn_swap_endian_4byte(str.slice(j, j + 8)) + str.slice(j + 8);
    }
    ...
    if (wordset.prefix_len > 0) {
        out.push(out[this.mn_get_checksum_index(out, wordset.prefix_len)]);
    }
    return out.join(' ');
}
